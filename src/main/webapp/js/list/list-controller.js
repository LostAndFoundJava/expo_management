...
controller('ListController',['$rootScope', '$scope', '$state', 'ListService', '$window', '$stateParams', '$uibModal',
    function ($rootScope, $scope, $state, ListService, $window, $stateParams, $uibModal) {
        ...
        function init() {
            ...
            } else if ($state.is('list.condition.query')) {
                let strr = $stateParams.query;
                $scope.query(strr);
        ...
        $scope.imgUrl = function(url) {
            ...
            if ($('body').hasClass('body-small')) {
                return url + '!315-200' + '.jpeg'
            } else {
                return url + '!630-400' + '.jpeg'
            }
        ...
        $rootScope.$on('querying', function (event, args) {
            if (args.query) {
